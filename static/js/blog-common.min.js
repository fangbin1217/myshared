...
function OnUpdateComment(n) {
    var r, t, i;
    n ? (r = $("#comment_edit_id").html(),
        t = $("#tbCommentBody").val(),
        t = escapeHTML(t),
        t = t.replace(/\n/g, "<br/>"),
        t = t.replace(/\[quote\]/g, '<fieldset class="comment_quote"><legend>引用<\/legend>'),
        t = t.replace(/\[\/quote\]/g, "<\/fieldset>"),
        $("#comment_body_" + r).html(t + " <span style='color:red'>修改成功！<\/span>"),
        i = location.href,
    i.indexOf("#") > 0 && (i = i.substring(0, i.indexOf("#"))),
        location.href = i + "#" + r,
        ResetCommentBox(),
        ShowCommentMsg("修改成功！")) : alert("修改失败！")
}
...
function outFromAggHome() {
    ...
    $.ajax({
        url: getAjaxBaseUrl() + "RemoveFromSiteHome.aspx",
        data: JSON.stringify({
            postId: cb_entryId
        }),
        type: "post",
        ...
        contentType: "application/json; charset=utf-8",
        success: function(n) {
            n ? $("#site_editor_opt").html("<span style='color:red'>操作成功<\/span>") : $("#site_editor_opt").html("<span style='color:red'>操作失败<\/span>")
        },
        error: function() {
...
function loadBlogNews() {
    /*
    $.ajax({
        url: "https://www.cnblogs.com/gyrgyr/ajax/news.aspx",
        type: "get",
        dataType: "text",
        success: function(n) {
    ...
    */
    ...
    var n = "<!--done-->\n" +
        ...
        "\t\n" +
        "<div id=\"blog-news\">\n" +
        "    <div class=\"head_img\"><img width=\"170\" height=\"170\" alt=\"戈丫汝十级美颜啦\" src=\"https://pic.cnblogs.com/avatar/903183/20190808162322.png\" class=\"img_avatar\"  style=\"border-radius:35px\"><div></div></div>\n" +
        "\n" +
        ...
        "        昵称：\n" +
        "        <a href=\"https://home.cnblogs.com/u/gyrgyr/\">\n" +
        "            戈丫汝\n" +
        "        </a>\n" +
        "        <br />\n" +
        "        园龄：\n" +
        "        <a href=\"https://home.cnblogs.com/u/gyrgyr/\" title=\"入园时间：2016-03-04\">\n" +
        "            3年5个月\n" +
        "        </a>\n" +
        "        <br />\n" +
        "        粉丝：\n" +
        "        <a href=\"https://home.cnblogs.com/u/gyrgyr/followers/\">\n" +
        "            32\n" +
        "        </a>\n" +
        "        <br />\n" +
        "        关注：\n" +
        "        <a href=\"https://home.cnblogs.com/u/gyrgyr/followees/\">\n" +
        "            7\n" +
        "        </a>\n" +
        "        <div id=\"p_b_follow\"></div>\n" +
        "        <script>getFollowStatus('5931d2fb-c4e1-e511-9fc1-ac853d9f53cc');</script>\n" +
        "    </div>\n" +
        "</div>\n" +
        "</div>\n" +
        "\n";
...
function blogCommentManager() {
    ...
    this.PostNewComment = function() {
        ...
        $.ajax({
            ...
            success: function(n) {
                if (n)
                    n.IsSuccess ? (ShowCommentMsg("感谢您的回复:) 服务器端执行耗时" + n.Duration + "毫秒"),
                        $("#tbCommentBody").val(""),
                        $("#divCommentShow").html($("#divCommentShow").html() + n.Message),
                        cb_mathjax_render("#divCommentShow")) : ShowCommentMsg(n.Message),
                        $("#btn_comment_submit").removeAttr("disabled");
                else
                    ShowCommentMsg("抱歉！评论提交失败！请与管理员联系(contact@cnblogs.com)。"),
                        $("#btn_comment_submit").removeAttr("disabled")
            },
            error: function(n, t) {
                n.status == 500 ? ShowCommentMsg("抱歉！发生了错误！麻烦反馈至contact@cnblogs.com") : n.status > 0 ? ShowCommentMsg("抱歉！评论提交失败！错误码：" + n.status + " 错误信息：" + n.responseText) : ShowCommentMsg("抱歉！评论提交失败！xhr.status: " + n.status + ", textStatus: " + t);
                $("#btn_comment_submit").removeAttr("disabled")
            }
        })
    }
            ...
            success: function(t) {
                if (t)
                    t.IsSuccess ? (ShowCommentMsg("修改成功"),
                        ...
                        commentManager.ResetCommentBox()) : ShowCommentMsg(t.Message);
                else
    ...
    this.renderComments = function(r, u, f) {
        ...
        e = !1;
        o = 0;
        typeof f != "undefined" && (o = f,
            e = !0);
        !e && r == 0 && location.hash && /^#\d+$/g.test(location.hash) && (e = !0,
                ...
                t ? ($("#post_comment_count").html(t),
                    t > 0 ? $.ajax({
                        ...
                        success: function(t) {
                            ...
                            e ? n("#" + o) : (location.hash == "#!comments" || location.hash == "#comments_pager_top") && n("#!comments");
                    ...
                    }) : $("#blog-comments-placeholder").html("")) : $("#blog-comments-placeholder").html("");
                i();
                cb_CodeHighlight();
...
function GetQuoteComment(n) {
    ...
    $(i).length && $(i).remove();
    t = $("#comment_body_" + n).html().trim();
    t = t.replace(/\n/g, "");
    t = t.replace(/<br\/?>/ig, "\n");
    t = t.replace(/<fieldset class=\"comment_quote\">((\w|\W)*?)<\/fieldset>/ig, "[quote]$1[/quote]");
    t = t.replace(/<[^>]*>/g, "");
    t.length > 300 && (t = t.replace(/<fieldset class=\"comment_quote\">((\w|\W)*?)<\/fieldset>/ig, "[quote]$1[/quote]"),
        t = t.substring(0, 300) + "...");
    t.length > 0 && (t = "[quote]\n" + t + "\n[/quote]\n");
    ...
    $.getScript(location.protocol + "//common.cnblogs.com/script/encoder.js", function() {
        ...
        $("#tbCommentBody").focus().val($("#tbCommentBody").val() + "@" + r + "\n" + t);
        $("#tip_comment").html("")
    })
}
...
var JSON, accountBaseAddress, currentDelElement, currentCommentID, insertUBB;
JSON || (JSON = {}),
    function() {
        "use strict";
        function i(n) {
